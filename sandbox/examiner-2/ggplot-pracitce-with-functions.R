make_tile_practice <- function(
  ...
  # create values for testing and development
  ...
  ##-##-##-##-##
  # graph the lables - LEFT panel of the display
  g_labels_left <- l$labels_long %>% 
    dplyr::filter(!agg_level == "PROV") %>% 
    # dplyr::filter(agg_level %in% c("HA", "HSDA")) %>% # another way to remove PROV ! = negate
    ggplot2::ggplot(
      aes(
        x = ""
        ,y = label_hsda
  ...
  g_values_right <- l$values_long %>% 
    ...
    geom_tile(aes(fill = censor_activated)) + #show.legend = FALSE - also remove legend
    # g <- g + geom_tile(aes(fill = value)) # aes mapps onto the values 
    # where fill outside does the same thing to all timles
    geom_text(aes(label = value)) + 
    facet_grid(. ~ agg_level) + 
  ...
  # g <- g + labs(x = NULL, y = NULL) # another way to remove the x and y axis label 
  # g_values_right
  ##-##-##-##-##
